import Link from 'next/link'
import React from 'react'
import Logo from './Logo'
import { useRouter } from 'next/router'
import {LeetcodeIcon,GithubIcon, LinkedInIcon, TwitterIcon , SunIcon, MoonIcon } from './Icons'
import {motion} from 'framer-motion'
import useThemeSwitcher from './hooks/useThemeSwitcher'

const CustomLink = ({href,title,tar,className=""})=>{
    const router = useRouter();
    return(
        <Link href={href} target={tar} className={`${className} relative group`}>
            {title}
            <span className={`h-[2px] inline-block bg-dark absolute left-0 -bottom-0.5
            group-hover:w-full transition-[width] dark:bg-light ease-in duration-300 ${router.asPath===href ? 'w-full' : 'w-0'}
            `}
            >&nbsp;</span>
        </Link>
    )
}

const Navbar = () => {

    const [mode ,setMode] = useThemeSwitcher()

  return (
    <header
    className='w-full px-32 py-8 font-medium flex items-center justify-between dark:text-light '>
        <nav>
            <CustomLink href="/" title="Home" className='mr-4'/>
            <CustomLink href="/about" title="About" className='mx-4'/>
            <CustomLink href="/projects" title="Projects" className='mx-4'/>
            <CustomLink href="mailto:chowdrisakthivel@gmail.com" title="Contact" target="_blank" className='ml-4'/>
        </nav>
        <nav className='flex items-center justify-center flex-wrap'>
        <motion.a href="https://twitter.com/itz_me_chow3" target={"_blank"} whileHover={{y:-2}} whileTap={{scale:2}} className='w-6 mr-3' ><TwitterIcon/></motion.a>
        <motion.a href="https://leetcode.com/ChowdriSakthivel/" target={"_blank"} whileHover={{y:-2}} whileTap={{scale:2}} className='w-6 mx-3' ><LeetcodeIcon/></motion.a>
        <motion.a href="https://github.com/ChowdriS/" target={"_blank"} whileHover={{y:-2}} whileTap={{scale:2}} className='w-6 mx-3'><GithubIcon/></motion.a>
        <motion.a href="https://www.linkedin.com/in/chowdri-sakthivel/" target={"_blank"} whileHover={{y:-2}} whileTap={{scale:2}} className='w-6 ml-3'><LinkedInIcon/></motion.a>

        <button
        onClick={() => {
            setMode (mode === "light" ? "dark":"light")
        }} 
        className={`ml-6 flex items-center justify-center rounded-full p-1 
        
        `}>
        {
        mode === "dark" ?
        <SunIcon className={"fill-dark"} />
        : <MoonIcon className={"fill-dark"} />
        }
        </button>

        </nav>
        <div className='absolute left-[50%] top-2 translate-x-[-50%]'>
            <Logo/>
        </div>
    </header>
  )
}

export default Navbar